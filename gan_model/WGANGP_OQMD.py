...
def Generator(noise, is_training, batch_size):
	...
	bn_1 = tf.reshape(tf.nn.relu(bn_1), [-1, 6, 4, 256])

	dec_2 = decnn2d(
				input = bn_1, 
				filter_shape = [5, 5, 128, 256], 
				output_shape = [batch_size, 11, 8, 128], 
				strid_shape = [1, 2, 2, 1], 
				padding = "SAME", 
				scope = "decnn_2", 
				name = "decnn2",
	...
	dec_4 = decnn2d(
				...
				output_shape = [batch_size, 22, 8, 64], 
				strid_shape = [1, 2, 1, 1], 
				padding = "SAME", 
				scope = "decnn_4", 
				name = "decnn4",
	...
	dec_6 = decnn2d(
				input = tf.nn.relu(bn_5), 
				filter_shape = [3, 3, 32, 64], 
				output_shape = [batch_size, 43, 8, 32], 
				strid_shape = [1, 2, 1, 1], 
				padding = "SAME", 
				scope = "decnn_6", 
				name = "decnn6",
	...
	output = tf.nn.sigmoid(dec_8, name = "output")
	return output
...
def Discriminator(inputs, is_training):
	...
	axis_1, axis_2  = bn_7.get_shape()[1], bn_7.get_shape()[2]
	output = linear(
					input = LeakyReLU(
							tf.reshape(bn_7, [-1, axis_1*axis_2*256])
						),
					output_dim = 1, 
					scope = "linear",
					name = "output"
